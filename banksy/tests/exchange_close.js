...
describe("exchange close", () =>{
    ...
    const nftSupply = new anchor.BN(100);
    ...
    it("create a exchange", async() => {
      ...
      const nft = await createNftAccount(nftProgram, uri, nftSupply, seller); // create NFT
      ...
      assert.ok(nftAccount.supply.toNumber() == nftSupply); // check the nft supply
    ...
    it("close exchange", async() => {
      ...
      let sellerItemAccount = await findUserAccount(nftProgram, exchangeAccount.seller, exchangeAccount.item);
      {
        const nftReciverAccount = await nftProgram.account.userAccount.fetch(sellerItemAccount); // get nft buyer account
      ...
      assert.ok(nftReciverAccount.amount.toNumber() == nftSupply.toNumber()); // check if the nft is transfered
